...
int read_pgm_hdr(FILE *fp, int *nrows, int *ncols) {
  ...
  if (skipcomment(fp) == EOF || fscanf(fp, "%2s", filetype) != 1 ||
      strcmp(filetype, "P5") || skipcomment(fp) == EOF ||
      fscanf(fp, "%d", ncols) != 1 || skipcomment(fp) == EOF ||
      fscanf(fp, "%d", nrows) != 1 || skipcomment(fp) == EOF ||
      fscanf(fp, "%d%*c", &maxval) != 1 || maxval > 255)
    return (-1);
  else
    return (0);
}
...
void **matrix(int nrows, int ncols, int first_row_coord, int first_col_coord,
              int element_size) {
  void **p;
  int alignment;
  long i;
...
/* The "reflect" funcstion will then fill in those empty */
...
/*--------------------------------------------------------------------------------------*/
void nms(float **anglec, float **magc, float **dd_image, int nrows, int ncols,
         int thresh, int thresh2) {

  /* PERFORMS NON-MAXIMUM SUPRESSION TO DETECT EDGES */

  int c, edgepoints1 = 0, edgepoints2 = 0, last = 0, nochange, row, col, count,
          run;
  FILE *fpy;
  float angle, maxm;
  int ax_pos, ay_pos, r1, c1, r2, c2, edgecount, r3, c3, r4, c4, i, j, r,
      count_points = 0;
  ...
  printf(" Performing Non_maximum-Supression\n");
   ...
   edgemap = (unsigned char **) matrix(nrows+2,ncols+2,-1,-1,sizeof(unsigned char));
        ...
        for (j = 0; j < nrows; j++) {
          ...
          for (row = i - 1; row <= i + 1; row++) {
            for (col = j - 1; col <= j + 1; col++) {
              ...
              if ((dd_image[i][j] * dd_image[row][col] <
                   (fabs(dd_image[i][j]) <= fabs(dd_image[row][col])))))
                count++;
            }
          }
          if (count >= 1) {
...
int gauss(unsigned char **xc, int nrows, int ncols, float sigma, int scale) {
  ...
  gauss_filter =
      (float **)matrix(grows, gcols, -2 * scale, -2 * scale, f(float));
  printf("ALLOCATED MEMORY\n");

  set = -scale;
  /*  ALLOCATE MEMORY FOR GAUSS SMOOTH */
  printf(" ALLOCATE MEMORY FOR GAUSS SMOOTH\n");
  gauss_smooth = (float **)matrix(nrows - 4 * set, ncols - 4 * set, 2 * set, ,
                                  sizeof(float));
  ...
  for (i = 0; i < nrows; i++) {
    for (j = 0; j < ncols; j++) {
      ...
      for (c = set; c <= -set; c++) {
        for (r = set; r <= -set; r++) {
          gauss_smooth[i][j] =
              gauss_smooth[i][j] + (float)xc[j + r] * gauss_filter[c][r];
        }
      }
    }
  }
...
void canny(unsigned char **xc, int nrows, int ncols, int thresh, int h2,
           float sigma, int scale) {

  int r, c, i, j, set, edgepoints2, last, nochange = 1, edgecount, check, count,
                                          row, cn, t1, t2;
  int count_points = 0;
  ...
  set = scale;
  fx = (float **)matrix(nrows + 4 * scale, ncols + 4 * scale, -2 * scale,
                        -2 * scale, f(float));
  fy = (float **)matrix(nrows + 4 * scale, ncols + 4 * scale, -2 * scale, ale,
                        sizeof(float));
  ...
  gradient = (float **)matrix(nrows + 2 * set, ncols + 2 * set, -2 * set,
                               -2 * set, f(float));
   dd_image = (float **)matrix(nrows + 4 * set, ncols + 4 * set, -2 * set,
                               -2 * set, f(float));
   anglec = (float **)matrix(nrows + 4 * set, ncols + 4 * set, -2 * set,
                             -2 * set, f(float));
   ...
   for (i = 0; i < ncols; i++) {
     for (j = 0; j < nrows; j++) {
       ...
       fxy[i][j] = gauss_smooth[i - 1][j - 1] + gauss_smooth[i + 1][j - 1] -
                   _smooth[i + 1][j + 1] - gauss_smooth[i - 1][j - 1];
       fxx[i][j] =
           gauss_smooth[i][j + 2] - 2 * gauss_smooth[i][j] + _smooth[i][j - 2];
       fyy[i][j] =
           gauss_smooth[i + 2][j] - 2 * gauss_smooth[i][j] + _smooth[i - 2][j];
...
int main(int argc, char **argv) {
  ...
  while (--argc > 0 && **(argv + 1) == '-')
    ...
    case 't':
      ...
      if (str[0] <= '9' && str[0] >= '0') {
        ...
        error("Usage:edge {-roberts,-prewitt,-sobel,-frei} [-skipNMS] "
              "hresh_low thresh_high] img ");
        exit(0);
      }
    ...
    default:
      error("Usage:edge {-roberts,-prewitt,-sobel,-frei} [-skipNMS] [-t h_low "
            "thresh_high] img ");
      break;
  ...
  x = (unsigned char **)matrix(nrows + 2 * scale, ncols + 2 * scale, -scale, e,
                               sizeof(char));
