...
CImg<unsigned char> spherical_warp::run_warping() {
    ...
    int projection_width = src.width();
    int projection_height = src.height();

    double r = (projection_width / 2.0) / tan(angle_unit * M_PI / 180.0);

    cimg_forXY(src, x, y) {
            double x_diff = x - projection_width / 2;
            double y_diff = y - projection_height / 2;
            ...
            if (src_x + projection_width / 2 >= 0 && src_x + projection_width / 2 < src.width()
                ...
                for (int z = 0; z < src.spectrum(); z++) {
                    result(x, y, z) = utils::interpolate(src, src_x + projection_width / 2, src_y + projection_height / 2, z);
                }
            }
