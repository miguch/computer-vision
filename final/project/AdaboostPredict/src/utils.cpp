...
void utils::saveResultToOneCSV(const char* filename, const std::vector<std::vector<std::vector<int>>>& list) {
    std::ofstream fs(filename);
    fs << "filename,studentID,phone,citizenID" << std::endl;
    for (int i = 0; i < list.size(); i++) {
        fs << i << ".jpg,";
        if (list[i].size() >= 1) {
            for (auto digit: list[i][0]) {
                if (digit == -1) fs << 'X';
                else fs << digit;
            }
